package ru.kpfu.dm.modul;

import org.w3c.dom.Document;
import org.w3c.dom.Element;
import org.w3c.dom.Node;
import org.w3c.dom.NodeList;
import ru.kpfu.dm.entity.GroupChild;
                ...
                if (nNode.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    for (int j = 0; j < nList2.getLength(); j++) { // Находим цену из offers.xml
                        ...
                        if (eElement2.getElementsByTagName("Ид").item(0).getTextContent().equals(eElement.getElementsByTagName("Ид").item(0).getTextContent())) {
                            ...
                            float price = (float) Double.parseDouble(eElement2.getElementsByTagName("ЦенаЗаЕдиницу").item(0).getTextContent());
                            System.out.println("Цена : " + price);
                            String currency = eElement2.getElementsByTagName("Валюта").item(0).getTextContent();
                            ...
                            products.add(new Product(article, name, stock, price, currency, productGroup));
